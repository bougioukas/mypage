...
hist(T_perm, breaks = 50, col = rgb(0.8, 1, 0.8), 
     ...
     xlab = "Test Statistic", xlim = c(-15, 15))
```
...
The extreme values of T statistic, which are unlikely under the null hypothesis, are highlighted in a light red color. The upper tail corresponds to $T \ge T_{\text{obs}}$, and the lower tail corresponds to $T \le -T_{\text{obs}}$.
...
text(T_obs, max(h$counts) * 0.98, paste0("T_obs = ", T_obs), 
...
In this example, the results are highly consistent. Both the exact Permutation Test ($p = 0.0179$) and the two-sample T-test ($p = 0.0192$) yield exceptionally close p-values.
