...
#| fig-width: 8
#| fig-height: 5
#| fig-cap: "Perceptron learning curve showing error reduction over epochs"
...
p <- ggplot() +
  ...
  geom_point(data = anim_data_extended, 
             ...
             shape = 21, size = 1.5, color = "black", stroke = 1.5) +
  ...
  labs(x = "Sepal Length", y = "Petal Length",
    ...
    subtitle = "Weights [w0, w1, w2]: {anim_data_extended$weights_text[as.integer(closest_state)]}\nAccuracy: {round(anim_data_extended$current_acc[as.integer(closest_state)], 1)}% | Step: {as.integer(closest_state)}"
  ) +
...
# Final Boundary Plot
plot(X[, 1], X[, 2], col = ifelse(y_vector == 1, "blue", "orange"), pch = 19, cex = 1.2,
     xlab = "Sepal Length (x1)", ylab = "Petal Length (x2)",
     main = "Final Decision Boundary")

x1_range <- seq(min(X[, 1]) - 0.5, max(X[, 1]) + 0.5, length.out = 100)
# Rearranged: x2 = -(w0 + w1*x1) / w2
x2_boundary <- -(w_final[1] + w_final[2] * x1_range) / w_final[3]
lines(x1_range, x2_boundary, col = "black", lwd = 2.2)

# Improved Label: Uses '+' sign for positive weights automatically
eq_text <- sprintf("%.2f + %.2f*x1 + %.2f*x2 = 0", 
                   w_final[1], w_final[2], w_final[3])
# Clean up double signs like "+ -" if necessary
eq_text <- gsub("\\+ -", "- ", eq_text)

mtext(eq_text, side = 3, line = -1, cex = 1.1, font = 3)

legend("topleft", legend = c("Versicolor (+1)", "Virginica (-1)", "Boundary"), 
       col = c("blue", "orange", "black"), 
       pch = c(19, 19, NA), lty = c(NA, NA, 2), cex = 1.1)
```
